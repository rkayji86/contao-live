...
class getContao_Filesystem_DbafsManagerService extends Contao_ManagerBundle_HttpKernel_ContaoKernelProdContainer
{
    ...
    public static function do($container, $lazyLoad = true)
    {
        ...
        $a = new \Contao\CoreBundle\Filesystem\Dbafs\DbafsFactory(($container->services['doctrine.dbal.default_connection'] ?? $container->getDoctrine_Dbal_DefaultConnectionService()), ($container->services['event_dispatcher'] ?? $container->getEventDispatcherService()));

        $b = $a->__invoke(($container->privates['contao.filesystem.virtual.files'] ?? $container->load('getContao_Filesystem_Virtual_FilesService')), new \Contao\CoreBundle\Filesystem\Dbafs\Hashing\HashGenerator('md5', true), 'tl_files');
        $b->useLastModified(true);
        $b->setDatabasePathPrefix('files');
        $c = $a->__invoke(($container->privates['contao.filesystem.virtual.notification_center_bulky_items_storage'] ?? $container->load('getContao_Filesystem_Virtual_NotificationCenterBulkyItemsStorageService')), new \Contao\CoreBundle\Filesystem\Dbafs\Hashing\HashGenerator('md5', false), 'tl_nc_bulky_items');
        $c->useLastModified(false);

        $instance->register($b, 'files');
        $instance->register($c, 'notification_center/bulky_items');
